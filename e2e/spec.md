...
| お気に入り追加 | `button[aria-label="お気に入りに追加"]` |
| お気に入り削除 | `button[aria-label="お気に入りから削除"]` |
...
| 11 | お気に入り登録/解除 | 両ページ | FavoriteButton | `POST/DELETE /api/v1/favorites` |
...
| ID | シナリオ | 前提条件 | 手順 | 期待結果 |
|----|---------|---------|------|---------|
| FA-1 | お気に入り未登録の初期状態 | テストデータ投入済み | 一覧ページで video-1 のお気に入りボタンを確認 | `aria-label="お気に入りに追加"` のボタンが表示され、ハートアイコンが塗りつぶされていない |
| FA-2 | お気に入りに追加（一覧ページ） | FA-1 の状態 | video-1 の「お気に入りに追加」ボタンをクリック | ボタンの `aria-label` が「お気に入りから削除」に変わり、ハートが塗りつぶされる |
| FA-3 | お気に入りから削除（一覧ページ） | FA-2 の状態 | video-1 の「お気に入りから削除」ボタンをクリック | ボタンの `aria-label` が「お気に入りに追加」に戻る |
| FA-4 | お気に入りに追加（詳細ページ） | video-1 の詳細ページ | 「お気に入りに追加」ボタンをクリック | ボタンの `aria-label` が「お気に入りから削除」に変わる |
| FA-5 | お気に入り状態の永続化 | FA-2 実行後 | ページをリロード | video-1 が引き続きお気に入り状態（`aria-label="お気に入りから削除"`）で表示される |
| FA-6 | お気に入りボタンのクリックでページ遷移しない | テストデータ投入済み、一覧ページ | video-1 のお気に入りボタンをクリック | 一覧ページのまま遷移しない（`e.preventDefault()` と `e.stopPropagation()` が効いている） |
| FA-7 | 詳細ページでのお気に入り状態が一覧に反映される | video-1 の詳細ページでお気に入り追加後 | 一覧ページに戻る | video-1 がお気に入り状態で表示される |
